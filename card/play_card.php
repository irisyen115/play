...
//接著抽出第一章牌
$points = $_SESSION['card'][$_SESSION['pos']];



//接著在把這些牌的點數存進session['points']
$_SESSION['pos'] = $_SESSION['pos']+1;
echo "你抽了".$_SESSION['pos']."張"."<br/>";
array_push($_SESSION['points'],$points);
echo "你拿到了這些"."<br/>";
for ($i = 0;$i < count($_SESSION['points']);$i++){
    $p = ($_SESSION['points'][$i] % 13) +1;
    if (floor($_SESSION['points'][$i] / 13) == 0){
        echo "黑桃".$p."<br/>";
    }

    elseif (floor($_SESSION['points'][$i] /13) == 1){
        echo "紅心".$p."<br/>";
    }
    
    elseif (floor($_SESSION['points'][$i] /13) == 2){
        echo "方塊".$p."<br/>";
    }
    
    elseif (floor($_SESSION['points'][$i] /13) == 3){
        echo "梅花".$p."<br/>";
    } 
}
